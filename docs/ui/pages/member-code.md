...
| 状态 | 进入条件 | 页面表现 | 恢复路径 |
|---|---|---|---|
| 已登录正常态 | 登录校验通过且会员码生成成功 | 顶部导航栏、完整会员码图片、绑定提示文案 | 30 秒自动刷新 |
...
- 白色卡片中应展示完整二维码图片，不应看到可见 canvas，不应只显示左上角 1/4，也不应被卡片裁切。
- 页面内容未超过屏幕时不应出现无意义滚动条；主区域高度由 `PageLayout` 处理，页面不再自行硬算 `100vh`。
...
- 移除页面主区域自身的 `100vh / calc(100vh - header)` 硬编码高度，避免和 `PageLayout` 的 header spacer 叠加导致无意义滚动。
- 背景承接到 `PageShell` / `PageLayout` class，主区域只保留内容自身间距。

### v0.5

- 修复隐藏 canvas 与 Taro `px` 转 `rpx` 尺寸不一致导致的二维码左上角 1/4 截取问题。
- 明确二维码生成位图尺寸使用微信真实像素，页面展示尺寸使用 750 设计稿 `rpx`。

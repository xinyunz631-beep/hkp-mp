...
- 用户中途发来的问题、提醒、补充条件，默认视为主线任务上的约束更新，不单独中断当前阶段执行。
- 只有用户明确说“停”“暂停”“先做别的”或明确改主任务时，才允许切出当前主线计划。
- 餐饮 `pkg-dining` 和分销链路 UI 仍按暂缓策略执行，除非用户明确点名恢复。
- 商用级补完不再把 `implemented` 视为最终完成；页面必须达到 `commercial-ready`，即 UI、交互、状态、mock 数据、登录和微信工具验收路径闭环。
- 当前先以“门票预定 + 门票确认订单”为样板页，沉淀交互矩阵、状态矩阵、微信 API 封装、NutUI 二次封装和本地订单草稿流转。

## 页面结构

- 主包：`home` 已实现；`park` 做乐园聚合入口；`member` 做我的入口，原 `profile` 路径已合并删除。
- `pkg-mall`：商城首页、搜索、列表、详情、规格弹层、购物车、收藏、赠品选择。
- `pkg-ticket`：乐园详情、门票预定、日期选择、优惠券弹层、门票确认订单。
- `pkg-hotel`：酒店详情、入住人弹层、入住日期、房型详情、酒店确认订单。
- `pkg-dining`：餐饮首页、商家详情、套餐选择、餐饮确认订单。当前阶段仅保留页面基础框架和 service/mock 入口，UI 等待用户明确通知后再做。
- `pkg-order`：订单列表、详情、售后、退款、评价、物流、地址。
- `pkg-member`：优惠券、分享收益、收益明细、邀请明细、提现。其中分销链路 `share-rule/share/share-income/share-invite/withdraw/withdraw-records` 当前仅保留基础框架，UI 等待用户明确通知后再做。

## 阶段顺序（调整后）

1. Phase 3：优先完成商城闭环。
2. Phase 4：先完成票务和酒店闭环；餐饮 `pkg-dining` 只保留基础框架，不做 UI 还原。
3. Phase 5：完成订单、售后、地址、评价、物流。
4. Phase 6：按需推进会员基础页与优惠券；分销链路只保留基础框架，不做 UI 还原。
5. Phase 7：补齐状态页、空态、错误态和整体验收。
6. 餐饮 UI、分销 UI 只有用户明确通知后才进入实现。
...
- `commercial-ready` 页面文档必须记录交互矩阵、状态矩阵和微信开发工具验收清单；页面内所有可见可点元素都必须有明确业务结果。
...
- `member` 我的入口：我的订单、地址管理、售后记录、优惠券。
...
- 样板链路：门票预定 -> 提交时登录拦截 -> 创建本地订单草稿 -> 门票确认订单 -> 模拟支付 -> 写入本地订单中心 -> 订单详情。
...
- `2026-05-16`：Phase 3 先完成商城首页、商品列表、商品详情、购物车四个关键页面的首版 UI、mock 数据和购买跳转闭环。
- `2026-05-16`：Phase 3 补齐商城搜索、分类、收藏、赠品、推荐等辅助页面首版，商城分包页面流已完整可串。
- `2026-05-16`：计划调整：餐饮 `pkg-dining` 和分销链路 UI 默认暂缓，只保留基础框架，只有用户明确点名后再进入 UI 实现。
- `2026-05-16`：执行约束补充：用户中途插话默认只更新约束，不中断当前主线计划。
- `2026-05-16`：已切入 Phase 4，当前优先完成票务与酒店闭环；餐饮和分销继续按挂起策略执行。
- `2026-05-16`：Phase 4 已先完成票务链路首版，乐园详情、门票预定、门票确认订单已可跳转串联，下一步进入酒店链路。
- `2026-05-16`：Phase 4 已完成酒店链路首版，酒店首页、房间详情、酒店确认订单已可跳转串联，Phase 4 可进入收口。
- `2026-05-16`：已切入 Phase 5，当前优先完成订单、售后、地址、物流与评价闭环；餐饮和分销继续按挂起策略执行。
- `2026-05-16`：Phase 5 已先完成订单主链路首版，订单首页、订单详情、确认订单地址态已可跳转串联，下一步进入售后与地址。
...
- `2026-05-17`：进入商用级补完样板阶段，门票预定和门票确认订单升级为 `commercial-ready`；新增微信动作封装、`DateSelectionPopup` 日期弹层、本地订单草稿和本地订单写入链路。

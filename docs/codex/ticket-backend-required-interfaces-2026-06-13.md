...
- 最新状态：2026-06-21 03:23 后端已复拉到 `origin/uat@2f57b7b docs(api): 回填门票批量日历 UAT 验证记录`，远端 head 只有 `main/prod/uat`，没有从 `uat` 最新提交后派生的票务接口新分支；`origin/uat@26fbc2b` 真实新增 `POST /api/bff/tickets/products/calendar-batch` 和内部 `POST /api/admin-config/mini-program/tickets/products/calendar-batch`，`2f57b7b` 已回填 UAT 已发布和匿名 401 鉴权证据。小程序已切换门票预定页首屏到批量接口，按 20 个商品一组分批。微信开发者工具自动化验收显示 `calendar-batch` 真实请求 2 次、旧单商品 `/calendar?startDate` 真实请求 0 次，页面“门票预订”断言通过。顺序创建探针 `ticket-after-calendar-batch-retry-20260621-01..08` 证明订单 `TKT20260621031528603F2551` 创建成功，`/pay` 返回 `PAYING/prepayPayNo=PAY2026062103152880ba2f508061/hasPaymentParams=true/paymentParamsAppId=wx72b9e08ce45d3e79`，付款前详情无 `ticketVouchers[]`，探针单已取消。TICKET-MP-P0-06 从“后端需修预下单”改为“保持稳定并进入真实支付成功后出票验收”；门票预定页 N+1 已完成首轮收口。
...
| TICKET-MP-P0-06 | `POST /api/bff/orders/{orderNo}/pay` | 门票恢复真实微信支付后，空支付参数会阻断小程序拉起支付；`94d87f6/b4fb3f7` 已补 AppID 字段透传，`b6c5258/94930b3` 已修复 AppID 大写导致的微信商户号不匹配；2026-06-21 03:23 在 `origin/uat@2f57b7b` 复跑探针，`/pay` traceId=`ticket-after-calendar-batch-retry-20260621-06` 返回 `PAYING/prepayPayNo=PAY2026062103152880ba2f508061/hasPaymentParams=true/paymentParamsAppId=wx72b9e08ce45d3e79`。 | `PENDING_PAYMENT/PAYING` 门票订单必须持续稳定返回 `prepay.payNo` 和 `prepay.paymentParams/payParams`，并保持小程序 AppID 原始大小写；非待支付状态重复调用返回 `ORDER_PAYMENT_NOT_ALLOWED`；支付成功后必须推动 order-service 出票并在订单详情返回 `ticketVouchers[]`。 |
| TICKET-MP-P0-09 | `POST /api/bff/tickets/products/calendar-batch` | 门票预定页首屏此前是 `products + N * calendar`，当前真实商品约 35 个时会产生跨商品 N+1。 | 2026-06-21 `origin/uat@26fbc2b/2f57b7b` 已交付并发布；小程序已切换为 `products + calendar-batch`，按后端单次最多 20 个商品分批。微信开发工具验收当前为 2 次 `calendar-batch`、0 次旧单商品日历；远端复核只有 `main/prod/uat`，没有新派生票务接口分支。后续如果后端提供页面级 `booking-page`，可再降为 1 个页面聚合请求。 |
...
- 2026-06-21 03:23 `origin/uat@26fbc2b/2f57b7b` 新增并发布门票批量库存日历接口；本轮复核远端只有 `main/prod/uat`，没有从 `uat` 最新提交后派生的票务接口新分支。小程序已完成适配：`src/pkg-ticket/services/ticket-api.ts` 新增 `fetchBffTicketCalendarBatch`，`src/pkg-ticket/services/ticket-booking.ts` 改为 20 个商品一批请求 `calendar-batch`，并把商品级日期摘要作为首屏可售性展示，SKU 最终价量继续交给 `quote/confirm` 校验。`mp-run-check` 验收显示 `calendar-batch` 2 次、旧 `/calendar?startDate` 0 次；`yarn typecheck`、`yarn check:page-convention`、`yarn check:package-boundary` 均通过。顺序创建探针 `ticket-after-calendar-batch-retry-20260621-01..08` 证明支付前链路稳定，订单 `TKT20260621031528603F2551` 已取消。当前 P0 顺序不变：真实微信支付成功后出票、真实支付退款、后台票码实例和核销流水同步。

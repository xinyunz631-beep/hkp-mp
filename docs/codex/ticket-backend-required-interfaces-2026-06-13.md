...
- 后端 `origin/uat@c79a546` 已交付并发布履约状态约束扩展、智游宝证件号来源修复、二维码 `img` 兼容、裸 base64 图片 data URI 归一、直出票退款跳过 pay-service 和已退票订单忽略迟到核销回调；`img` 会映射到既有 `ticketVouchers[].codeImage/qrImage`，小程序不新增 `rawFields.img` 运行时依赖。

...
- 后端 `origin/uat@cf7e4b5` 已交付并发布履约状态约束扩展、智游宝证件号来源修复、二维码 `img` 兼容、裸 base64 图片 data URI 归一、直出票退款跳过 pay-service、已退票订单忽略迟到核销回调，以及内部 `GET /api/order/thirdparty/zhiyoubao/orders/{orderCode}/check-status` 主动查询后同步 order-service 本地履约状态；`img` 会映射到既有 `ticketVouchers[].codeImage/qrImage`，小程序不新增 `rawFields.img` 运行时依赖。
...
- `cf7e4b5` 只证明内部主动查询入口会复用回调逻辑落 order-service 履约；当前小程序 `GET /api/bff/orders/{orderNo}` 详情链路没有自动调用该内部 check-status，3 秒静默轮询能否及时刷新仍依赖智游宝异步回调到达，或后端另有定时任务/后台动作主动调用该内部同步入口。

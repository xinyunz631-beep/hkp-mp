...
| TICKET-MP-P0-06 | `POST /api/bff/orders/{orderNo}/pay` | 门票恢复真实微信支付后，空支付参数会阻断小程序拉起支付；2026-06-19 13:25 和 16:44 UAT 曾返回微信预支付参数，但 17:08 创建订单成功后连续两次 `/pay` 回归 `INTERNAL_ERROR /api/pay/prepay failed`；17:35 后端 `94d87f6/b4fb3f7` 已补 AppID 字段透传。19:42 同步到 `origin/uat@b2a8a37` 后，票务 `products/calendar/quote` 三个读接口先 500，订单 `TKT20260619194203B06C3B9B` 的 `/pay` traceId=`ticket-after-b2a8a37-08` 仍失败。 | 后端必须先恢复票务读接口 200；`PENDING_PAYMENT/PAYING` 门票订单必须稳定返回 `prepay.payNo` 和 `prepay.paymentParams/payParams`；非待支付状态重复调用返回 `ORDER_PAYMENT_NOT_ALLOWED`；支付成功后必须推动 order-service 出票并在订单详情返回 `ticketVouchers[]`。 |
...
- 2026-06-19 19:42 后端 `origin/uat@b2a8a37` 只改 admin-config 会员补录手机号区号查重，不触达票务源码；探针 `ticket-after-b2a8a37-03..10` 显示 `GET /api/bff/tickets/products`、单商品 `/calendar` 和 `POST /api/bff/tickets/quote` 均 500，`orders/confirm` 与创建订单仍 200，订单 `TKT20260619194203B06C3B9B` 的 `/pay` traceId=`ticket-after-b2a8a37-08` 仍返回 `INTERNAL_ERROR /api/pay/prepay failed`，cleanup traceId=`ticket-after-b2a8a37-10` 已取消。当前 P0 顺序调整为先恢复票务 BFF 读接口，再恢复 pay-service/微信预下单稳定性。

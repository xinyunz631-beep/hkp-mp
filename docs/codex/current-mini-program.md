...
- 当前票务状态：门票预定页已切 `/api/bff/tickets/**`，支持快速通、草稿/待审核待上线展示、已发布库存加购、票种规则弹窗、0 元票和按 SKU 实名字段提交、无可订票种空态；小程序创建订单已把首位实名游客证件同步放入 `context.certificateNo`。2026-06-18 后端 `origin/uat@12ac189` 已解除履约表旧约束阻塞，把智游宝 `img` 映射到 `codeImage/qrImage`，并把裸 base64 图片归一为可直接渲染的 `data:image/*;base64,`；严格探针订单 `TKT202606181516448D71CD09` 已返回 `WAIT_USE + ticketVouchers[]`，订单详情可展示 `ticketCode/voucherCode/codeImage/qrImage`；同单标准化智游宝回调后订单详情已回读 `orderStatus=USED/ticketStatus=FULFILLED/usedNum=1`，订单详情 3 秒后台静默轮询读取路径成立，剩余验收是后台票码实例和核销流水一致性。
...
- 门票创建订单接口：`POST https://hellokitty-uat.yoursite.xin/api/bff/orders`，需小程序登录态 Bearer，不需要 HMAC 签名；`sceneType=TICKET` 创建成功后后端直接生成内部免支付流水并调用智游宝出票，响应 `order.orderStatus=WAIT_USE` 和 `order.ticketVouchers[]` 时前端直接进入订单详情；当前小程序会在 `context.certificateNo` 同步首位实名游客证件。2026-06-18 15:16 后端 `origin/uat@12ac189` 已通过严格探针，订单 `TKT202606181516448D71CD09` 返回 `WAIT_USE + ticketVouchers[]`，详情凭证含 `ticketCode/voucherCode/codeImage/qrImage`，二维码字段为可直接渲染的 `data:image/jpeg;base64,...`；剩余验收是核销后状态刷新。
...
- 订单已新增 `src/core/services/bff-order-api.ts`，覆盖旧提交兼容、统一订单确认、统一订单创建、支付、取消、退款、列表和详情 BFF 入口；酒店确认单、酒店下单支付、订单中心列表和订单详情已切真实接口；票务确认单、创建订单、创建即出票、`ticketVouchers` 展示已按后端真实链路接入，且创建请求已补 `context.certificateNo` 兼容当前后端证件读取；订单详情已补票务未终态 3 秒后台静默轮询。后端 `origin/uat@12ac189` 已解除履约表约束阻塞，把智游宝 `img` 映射到 `codeImage/qrImage`，并返回可渲染 data URI，小程序无需新增 `rawFields.img` 页面依赖；商城交易链路待后续按自身工作包切换。
...
- 2026-06-18 15:16 复拉后端 `origin/uat@12ac189 fix(order): 返回可渲染智游宝二维码`，本批补 `park_order_fulfillment` 票务状态约束、智游宝证件号来源、退票单号、二维码 `img` 字段兼容和裸 base64 图片 data URI 归一。`TICKET_PROBE_CREATE=1 TICKET_PROBE_STRICT=1 node scripts/probe-ticket-closure.mjs` 退出 0，`closure.closed=true`、`strictPass=true`，traceId=`ticket-closure-mqj62irg-01..06`；订单 `TKT202606181516448D71CD09` 进入 `WAIT_USE`，详情 `ticketVouchers[0]` 含 `ticketCode/voucherCode` 和可展示 `codeImage/qrImage`，二维码字段已是 `data:image/jpeg;base64,...`。随后同单标准化智游宝回调 traceId=`ticket-callback-mqj6ptam` 成功，订单详情 traceId=`ticket-callback-mqj6ptc6` 回读 `orderStatus=USED/ticketStatus=FULFILLED/usedNum=1/totalNum=1`。当前票务下单出票、订单详情券码展示和核销后详情状态刷新闭环已成立；下一段只剩有效后台登录态下复查票码实例和核销流水一致性。
## 登录体系目标
核心原则：
- `App` 不作为可见 UI 全局容器。

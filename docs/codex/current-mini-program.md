...
- 下单可用券接口：`GET https://hellokitty-uat.yoursite.xin/api/bff/promotion/coupons/available`，需小程序登录态；后端 `origin/uat@4047a42` 已核验，其中 `27c68ed` 补齐 `itemIds/skuIds/visitDate/checkInDate/checkOutDate`，必须读取与我的券同源的 `couponNo`，门票/酒店确认单选择后通过统一订单 `selectedCouponNos` 重新确认金额。

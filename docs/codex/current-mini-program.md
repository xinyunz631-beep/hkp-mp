...
- 2026-07-02 网易七鱼客服接入完成：`src/core/config/customer-service.ts` 已写入七鱼客服 `appKey=3a64f8e1cf82e1d4a22bee6f8dbca68a` 和小程序 `appId=wx72b9e08ce45d3e79`，`src/app.config.ts` 已声明微信插件 `qiyuSdk@1.4.7/provider=wxae5e29812005203f`，启动侧注册真实七鱼 adapter，首页、会员、商城商品详情和广告客服入口继续复用 `openCustomerService()`；每次打开客服前都会用当前会员资料调用七鱼 `_$setUserInfo` 覆盖访客身份，未登录时写空身份以避免沿用旧会员。

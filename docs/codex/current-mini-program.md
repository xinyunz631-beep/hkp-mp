...
- 当前票务状态：门票预定页已切 `/api/bff/tickets/**`，支持快速通、草稿/待审核待上线展示、已发布库存加购、票种规则弹窗、0 元票和按 SKU 实名字段提交、无可订票种空态；小程序创建订单已把首位实名游客证件同步放入 `context.certificateNo`。2026-06-18 后端 `origin/uat@cf7e4b5` 已解除履约表旧约束阻塞，把智游宝 `img` 映射到 `codeImage/qrImage`，把裸 base64 图片归一为可渲染 `data:image/*;base64,`，修复直出票 `BYPASS_` 退款跳过 pay-service，保护已退票订单忽略迟到核销回调，并让内部智游宝 `check-status` 主动查询后同步 order-service 履约状态。严格探针订单 `TKT202606181609268A8727B2` 已返回 `WAIT_USE + ticketVouchers[]`，退款 traceId=`ticket-refund-mqj81o00-01` 后详情 traceId=`ticket-refund-mqj81o00-02` 回读 `REFUNDED`，退款后迟到核销回调 traceId=`ticket-refund-callback-mqj8zgrl-01` 返回 `ZHIYOUBAO_ORDER_STATUS_CALLBACK_IGNORED` 且订单保持 `REFUNDED`，历史订单 `TKT202606181516448D71CD09` 标准化智游宝回调后详情已回读 `USED/FULFILLED/usedNum=1`；订单详情 3 秒后台静默轮询读取路径成立，但 BFF 详情不会自动调用内部 `check-status`，更快刷新仍依赖智游宝回调或后端主动同步任务。后台登录 traceId=`ticket-admin-closure-mqj7npe7` 已成功，但同单按订单号和真实票码查 `instances/verifications` 仍为空，实例详情返回 `ADMIN_TICKET_INSTANCE_NOT_FOUND`，剩余缺口是后台票码实例和核销流水未与 order-service 履约同步。
- 当前券/商城状态：优惠券链路已新增 `src/core/services/bff-coupon-api.ts`，我的券、领券中心领取、K 币余额/兑换、门票和酒店确认单可用券已切真实 BFF；2026-06-18 00:36 按后端 `origin/uat@4047a42` 复核，K 币兑换、后台发券和券包赠送已从源码层同步同一 `couponNo` 到 `promotion_member_coupon`，我的券已适配分页、来源和返还状态字段，门票和酒店可用券已适配 `available/unavailableReason/discountAmount` 以及 `itemIds/skuIds/visitDate/checkInDate/checkOutDate`；小程序优惠券 BFF 后端必补文档维护在 `admin-frontend/docs/codex/admin-api-requirements/mp-bff-requirements/`，旧 `docs/codex/apimust/` 只保留迁移指针；券/商城仍不能宣称完整闭环，因为缺目标 `couponNo` 严格探针、管理端券模板 `target_ids` 同步、真实已核销券退款返还样本，以及后台真实商城商品后的写链路验收。
- 最新券/商城复核：2026-06-18 12:36 已按后端 `origin/uat@e3138d9` 核验；`e3138d9` 仅影响 order-service 深大上报默认场所/仓库，不改变券或商城 BFF 契约。券链路仍以 `c379081` 前已核验的同源、过滤和幂等能力为准，`beb169c/c379081` 已修复商城展示与购物车读接口空态 500，最新商城探针 `mall-after-e3138d9-01..07` 均 200 空结构；券只读探针 `coupon-after-e3138d9-01..04` 均 200 但会员无券、无券包、K 币为 0。当前仍不能宣称完整闭环，因为缺目标 `couponNo` 严格探针、管理端券模板 `target_ids` 同步、真实已核销券退款返还样本验收，以及后台真实商城商品后的详情、加购、购物车写入、统一订单与用券验收。
...
- 门票创建订单接口：`POST https://hellokitty-uat.yoursite.xin/api/bff/orders`，需小程序登录态 Bearer，不需要 HMAC 签名；`sceneType=TICKET` 创建成功后后端直接生成内部免支付流水并调用智游宝出票，响应 `order.orderStatus=WAIT_USE` 和 `order.ticketVouchers[]` 时前端直接进入订单详情；当前小程序会在 `context.certificateNo` 同步首位实名游客证件。2026-06-18 16:58 后端 `origin/uat@cf7e4b5` 已通过严格探针，订单 `TKT202606181609268A8727B2` 返回 `WAIT_USE + ticketVouchers[]`；退款后详情回读 `REFUNDED`，退款后迟到核销回调已被忽略且订单保持 `REFUNDED`；内部 `GET /api/order/thirdparty/zhiyoubao/orders/{orderCode}/check-status` 主动查询后会同步 order-service 履约。核销后状态刷新已由订单 `TKT202606181516448D71CD09` 标准化回调验证，当前剩余缺口是后台票码实例和核销流水同步，以及后端是否把主动查询同步接入定时任务或 BFF/后台触发链路。
...
- 下单可用券接口：`GET https://hellokitty-uat.yoursite.xin/api/bff/promotion/coupons/available`，需小程序登录态；后端最新 `origin/uat@e3138d9` 已复核且无券接口变化，其中 `27c68ed` 补齐 `itemIds/skuIds/visitDate/checkInDate/checkOutDate`，`ad09fb4` 已补订单 confirm / promotion quote 候选目标校验；必须读取与我的券同源的 `couponNo`，门票/酒店确认单选择后通过统一订单 `selectedCouponNos` 重新确认金额。
...
- 订单已新增 `src/core/services/bff-order-api.ts`，覆盖旧提交兼容、统一订单确认、统一订单创建、支付、取消、退款、列表和详情 BFF 入口；酒店确认单、酒店下单支付、订单中心列表和订单详情已切真实接口；票务确认单、创建订单、创建即出票、`ticketVouchers` 展示已按后端真实链路接入，且创建请求已补 `context.certificateNo` 兼容当前后端证件读取；订单详情已补票务未终态 3 秒后台静默轮询。后端 `origin/uat@cf7e4b5` 已解除履约表约束阻塞，把智游宝 `img` 映射到 `codeImage/qrImage`，返回可渲染 data URI，让直出票退款跳过 pay-service 后推进为 `REFUNDED`，避免退款后迟到核销回调覆盖退款终态，并在内部主动查询智游宝核销状态后同步 order-service 本地履约；小程序无需新增 `rawFields.img` 页面依赖，也不能直接调用内部 order-service 同步入口；商城交易链路待后续按自身工作包切换。
...
- 2026-06-18 16:58 复拉后端 `origin/uat@cf7e4b5 fix(order): 主动同步智游宝核销状态`，在 `c79a546` 退款终态保护基础上新增内部主动查询同步：`GET /api/order/thirdparty/zhiyoubao/orders/{orderCode}/check-status` 调用智游宝查询后，会转换为标准回调请求并复用 order-service 本地履约同步逻辑，同时用 `USAGE_STATUS_REGRESSION` 保护旧查询结果不回退已核销状态。该提交没有新增小程序 BFF 路径；`GET /api/bff/orders/{orderNo}` 详情仍不自动调用内部 check-status。严格探针 `ticket-closure-mqj7yaw3-01..06` 已证明创建订单、`WAIT_USE` 和订单详情凭证，退款 traceId=`ticket-refund-mqj81o00-01/02` 已证明直出票退款回读 `REFUNDED`，迟到核销回调 traceId=`ticket-refund-callback-mqj8zgrl-01` 已证明退款终态保护；历史订单 `TKT202606181516448D71CD09` 标准化回调后详情已回读 `USED/FULFILLED/usedNum=1`。后台登录已成功但实例/流水仍空，当前缺口锁定为后台票码实例和核销流水未同步，以及后端是否把内部主动同步接入定时任务或后台触发链路。
## 登录体系目标
核心原则：
- `App` 不作为可见 UI 全局容器。

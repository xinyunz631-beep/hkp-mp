...
- 最新票务阻塞：2026-06-19 19:42 后端 `origin/uat@b2a8a37` 只改 admin-config 会员补录手机号区号查重，不触达票务源码；管理后台会员补录前端已传 `phoneCountryCode`，无需小程序或后台适配。探针 `ticket-after-b2a8a37-03..10` 显示 `GET /api/bff/tickets/products`、单商品 `/calendar` 和 `POST /api/bff/tickets/quote` 均 500，`orders/confirm` 与创建订单仍通，订单 `TKT20260619194203B06C3B9B` 为 `PENDING_PAYMENT`，但 `/pay` traceId=`ticket-after-b2a8a37-08` 仍返回 `/api/pay/prepay failed` 且无支付参数，探针单已取消。当前不能归因前端 AppID、后台库存、确认单字段或小程序模拟支付，后端需先恢复票务 BFF 读接口，再修 pay-service/微信预下单运行态。

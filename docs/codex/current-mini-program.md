...
- 最新券/商城复核：2026-06-19 已按后端 `origin/uat@2e44302` 复拉核验；`2e44302` 只改 pay-service 微信退款，不改变券或商城 BFF 契约。券链路仍以 `c379081` 前已核验的同源、过滤和幂等能力为准，`beb169c/c379081` 已修复商城展示与购物车读接口空态 500；商城探针 `mall-after-e3138d9-01..07` 均 200 空结构。2026-06-19 13:49 小程序已补商城确认单券选择、重新确认和创建订单传券；2026-06-19 本轮只读券探针 `coupon-goal-continue-20260619-01..04` 四个接口均 200，会员 `couponCount=0`、可用券 0、券包 0、K 币 0；CRM 展示入口 `coupon-crm-scan-20260619-01..06` 均 200，但缺 `templateNo/couponTemplateId/exchangeCode/couponQuantity`；候选领取 `coupon-claim-new-user-20260619-05`、`coupon-claim-member-discount-20260619-05` 和候选兑换 `coupon-kcoin-service-voucher-20260619-05`、`coupon-kcoin-gift-20260619-05` 均返回 500。当前仍不能宣称完整闭环，因为后端还需补 CRM 动作字段、稳定业务错误码、目标 `couponNo` 严格探针、管理端券模板 `target_ids`、真实已核销券退款返还样本，以及后台真实商城商品后的详情、加购、购物车写入、统一订单与用券验收。

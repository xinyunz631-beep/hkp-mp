...
- 当前票务状态：门票预定页已切 `/api/bff/tickets/**`，支持快速通、草稿/待审核待上线展示、已发布库存加购、票种规则弹窗、0 元票和按 SKU 实名字段提交、无可订票种空态；小程序创建订单已把首位实名游客证件同步放入 `context.certificateNo`。2026-06-18 后端 `origin/uat@c79a546` 已解除履约表旧约束阻塞，把智游宝 `img` 映射到 `codeImage/qrImage`，把裸 base64 图片归一为可渲染 `data:image/*;base64,`，修复直出票 `BYPASS_` 退款跳过 pay-service，并保护已退票订单忽略迟到核销回调。严格探针订单 `TKT202606181609268A8727B2` 已返回 `WAIT_USE + ticketVouchers[]`，退款 traceId=`ticket-refund-mqj81o00-01` 后详情 traceId=`ticket-refund-mqj81o00-02` 回读 `REFUNDED`，退款后迟到核销回调 traceId=`ticket-refund-callback-mqj8zgrl-01` 返回 `ZHIYOUBAO_ORDER_STATUS_CALLBACK_IGNORED` 且订单保持 `REFUNDED`，历史订单 `TKT202606181516448D71CD09` 标准化智游宝回调后详情已回读 `USED/FULFILLED/usedNum=1`；订单详情 3 秒后台静默轮询读取路径成立。后台登录 traceId=`ticket-admin-closure-mqj7npe7` 已成功，但同单按订单号和真实票码查 `instances/verifications` 仍为空，实例详情返回 `ADMIN_TICKET_INSTANCE_NOT_FOUND`，剩余缺口是后台票码实例和核销流水未与 order-service 履约同步。
...
- 门票创建订单接口：`POST https://hellokitty-uat.yoursite.xin/api/bff/orders`，需小程序登录态 Bearer，不需要 HMAC 签名；`sceneType=TICKET` 创建成功后后端直接生成内部免支付流水并调用智游宝出票，响应 `order.orderStatus=WAIT_USE` 和 `order.ticketVouchers[]` 时前端直接进入订单详情；当前小程序会在 `context.certificateNo` 同步首位实名游客证件。2026-06-18 16:38 后端 `origin/uat@c79a546` 已通过严格探针，订单 `TKT202606181609268A8727B2` 返回 `WAIT_USE + ticketVouchers[]`；退款后详情回读 `REFUNDED`，退款后迟到核销回调已被忽略且订单保持 `REFUNDED`。核销后状态刷新已由订单 `TKT202606181516448D71CD09` 标准化回调验证，当前剩余缺口是后台票码实例和核销流水同步。
...
- 订单已新增 `src/core/services/bff-order-api.ts`，覆盖旧提交兼容、统一订单确认、统一订单创建、支付、取消、退款、列表和详情 BFF 入口；酒店确认单、酒店下单支付、订单中心列表和订单详情已切真实接口；票务确认单、创建订单、创建即出票、`ticketVouchers` 展示已按后端真实链路接入，且创建请求已补 `context.certificateNo` 兼容当前后端证件读取；订单详情已补票务未终态 3 秒后台静默轮询。后端 `origin/uat@c79a546` 已解除履约表约束阻塞，把智游宝 `img` 映射到 `codeImage/qrImage`，返回可渲染 data URI，让直出票退款跳过 pay-service 后推进为 `REFUNDED`，并避免退款后迟到核销回调覆盖退款终态，小程序无需新增 `rawFields.img` 页面依赖；商城交易链路待后续按自身工作包切换。
...
- 2026-06-18 16:38 复拉后端 `origin/uat@c79a546 fix(order): 忽略已退票后的智游宝回调`，在 `6ecc3be` 直出票退款基础上新增退款终态保护。`TICKET_PROBE_CREATE=1 TICKET_PROBE_STRICT=1 node scripts/probe-ticket-closure.mjs` 对 `6ecc3be` 退出 0，`closure.closed=true`、`strictPass=true`，traceId=`ticket-closure-mqj7yaw3-01..06`；订单 `TKT202606181609268A8727B2` 进入 `WAIT_USE` 且详情有 1 张可用凭证。随后退款 traceId=`ticket-refund-mqj81o00-01` 成功，详情 traceId=`ticket-refund-mqj81o00-02` 回读 `REFUNDED`；DB 只读确认 `payStatus=SKIPPED/payReason=PAYMENT_BYPASSED`。对该退款单再打迟到核销回调 traceId=`ticket-refund-callback-mqj8zgrl-01`，返回 `ZHIYOUBAO_ORDER_STATUS_CALLBACK_IGNORED`，DB 只读确认 `order_status/fulfillment_status/refund_status` 均保持 `REFUNDED`，最新事件 reason=`REFUNDED_ORDER_TERMINAL`。历史订单 `TKT202606181516448D71CD09` 标准化回调后详情已回读 `USED/FULFILLED/usedNum=1`；后台登录已成功但实例/流水仍空，当前缺口锁定为后台票码实例和核销流水未同步。
## 登录体系目标
核心原则：
- `App` 不作为可见 UI 全局容器。

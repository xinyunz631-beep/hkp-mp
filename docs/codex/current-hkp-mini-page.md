...
- 更新时间：`2026-05-20`
- 当前阶段：`Phase Hotel Commercial - 酒店预订全链路商用闭环`
- 当前分支：`feature/hkp-mini-phase-9-ticket-checkout`
- 基线提交：`8c32509 feat: 完善商城搜索与分享链路`
- 最近阶段提交：`8c32509 feat: 完善商城搜索与分享链路`
...
2. 酒店首页需要重点验收：最终稿首屏还原、banner 滑动/预览、日期范围、入住人数弹层、筛选刷新、产品详情、预订进入确认订单。
3. 会员码页需要在微信开发工具重新编译后确认：顶部导航栏可见、左侧返回可用、白卡内展示完整二维码图片，不出现左上角 1/4 截取，不出现无意义滚动条。
4. 商城搜索页需要在微信开发工具重新编译后确认：输入文字后键盘搜索进入商品列表，清除按钮只清空输入不跳转，热门关键词仍可进入商品列表。
5. 地图入口需要在微信开发工具重新编译后确认：首页“导航至乐园”、门票预定页地址、乐园详情“园区地址”均应打开到安吉 Hello Kitty 乐园实际位置，而不是安吉县城旧偏移点。
6. 用户反馈不符合预期的点后，Codex 进入本批修复循环。
7. 验收反馈修完后继续做核心页面视觉/交互回归与缺口修复；餐饮和分销/提现继续最后处理。

...
- 富文本字段必须原样透传给项目既有富文本渲染组件；除非用户明确要求格式化、清洗、转义、包默认标签或二次排版，否则不得在 service、页面或组件里改写接口返回内容。
- 门票预订页、门票确认单、订单详情、入园凭证等所有“门票须知 / 预定须知 / 用票说明”只能渲染后端富文本字段原文，例如 `usageInstructionHtml`、`*Html`、`richText`；不得用 `notice`、`noticeText`、`refundRule`、`qualificationRule`、入园时间、入园地点、库存、日期等字段拼接或兜底成须知内容。
